...
// Sample listening materials with text-to-speech based content
...
That's my typical day! What about you? What do you usually do?`,
    level: 'A2' as Level,
    duration: 90,
    questions: [
      {
      ...
      {
        question: 'What time does the speaker finish work?',
...
The French food was absolutely delicious. I tried croissants, baguettes, and many different cheeses. My favorite was the chocolate mousse. I ate it almost every day!

I also took a boat tour on the Seine River. It was so romantic! We passed under many beautiful bridges and saw historic buildings on both sides.

The people in Paris were very friendly. Even though I don't speak French very well, many people tried to help me when I was lost.

I spent seven days in Paris, but it wasn't enough. There's so much to see and do. I definitely want to go back someday.`,
    level: 'B1' as Level,
    duration: 120,
    questions: [
      {
        question: 'Where did the speaker stay in Paris?',
        options: [
          'In a large hotel downtown',
          'In a small hotel near the Eiffel Tower',
          'In an apartment',
          'With friends'
        ],
        answer: 1
      ...
      {
        question: 'What famous painting did the speaker see?',
        options: ['The Starry Night', 'The Last Supper', 'Mona Lisa', 'The Scream'],
        answer: 2
      },
      {
        question: 'How long did the speaker stay in Paris?',
        options: ['Five days', 'Seven days', 'Ten days', 'Two weeks'],
        answer: 1
      }
    ]
  },
...
Global temperatures have risen by approximately 1.1 degrees Celsius since the pre-industrial era. While this might seem small, it has had significant consequences. Arctic ice is melting at an alarming rate, sea levels are rising, and extreme weather events are becoming more frequent.

The primary cause of climate change is the emission of greenhouse gases, particularly carbon dioxide from burning fossil fuels. Transportation, industrial processes, and electricity generation are major contributors.

However, there are reasons for optimism. Renewable energy sources like solar and wind power are becoming more affordable and efficient. Many countries have committed to reducing their carbon emissions through international agreements.

Individual actions also matter. Simple changes like using public transportation, reducing meat consumption, and minimizing waste can collectively make a significant impact.

The challenge ahead is substantial, but with global cooperation and technological innovation, we can work towards a more sustainable future. The decisions we make today will determine the world we leave for future generations.`,
    level: 'B2' as Level,
    duration: 150,
    questions: [
      {
        question: 'By how much have global temperatures risen since the pre-industrial era?',
        options: ['0.5 degrees', '1.1 degrees', '2 degrees', '3 degrees'],
        answer: 1
      },
      {
      ...
      {
        question: 'What does the speaker say about renewable energy?',
        options: [
          'It is too expensive',
          'It is becoming more affordable and efficient',
...
export function ListeningModule() {
  ...
  const filteredMaterials = listeningMaterials.filter(material =>
    ...
    <div className="space-y-6">
      ...
      {activeMaterial ? (
        <ListeningDetail
          ...
          levelColor={levelColors[activeMaterial.level]}
        />
      ) : (
        <>
          {/* Filters */}
...
// Listening Detail Component
function ListeningDetail({
  material,
  onBack,
  levelColor
}: {
  material: typeof listeningMaterials[0]
  onBack: () => void
  levelColor: string
}) {
  ...
  return (
    <div className="space-y-6">
      <Button variant="ghost" onClick={() => { stopSpeech(); onBack(); }}>
      ...
      {!showQuiz ? (
        <>
          ...
          <Button onClick={() => { stopSpeech(); setShowQuiz(true); }} className="w-full">
