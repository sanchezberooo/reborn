...
import { sampleGrammarRules, grammarCategories } from '../lib/data'
import type { GrammarRule, Level } from '../lib/types'
      ...
      <Tabs value={viewMode} onValueChange={(v) => setViewMode(v as 'rules' | 'practice')}>
        ...
        <TabsContent value="rules" className="mt-6">
          {activeRule ? (
            <GrammarRuleDetail 
              rule={activeRule} 
              onBack={() => setActiveRule(null)}
              levelColor={levelColors[activeRule.level]}
            />
          ) : (
                  ...
                  <CardContent className="pt-6">
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <span className="text-sm text-muted-foreground">
                        {rule.examples.length} ornek, {rule.exercises.length} egzersiz
                      </span>
...
// Grammar Rule Detail Component
function GrammarRuleDetail({ 
  rule, 
  onBack,
  levelColor 
}: { 
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-r from-primary/10 to-accent/10 border-primary/30">
        <CardContent className="pt-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-semibold">Formul</span>
          </div>
                ...
                <p className="text-lg">
                  {example.highlight ? (
                    <>
                      ...
                      <span className="bg-primary/30 px-1 rounded font-semibold">{example.highlight}</span>
              ...
              {rule.commonMistakes.map((mistake, index) => (
                <div key={index} className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="line-through text-destructive">{mistake.wrong}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Check className="h-4 w-4 text-green-500" />
                    <span className="text-green-400">{mistake.correct}</span>
                  </div>
      ...
      <Card>
        ...
        <CardContent>
          <GrammarExercises exercises={rule.exercises} />
        </CardContent>
      </Card>
    </div>
  )
...
function GrammarExercises({ exercises }: { exercises: GrammarRule['exercises'] }) {
  ...
  if (exercises.length === 0) {
    return <p className="text-muted-foreground">Bu kural icin egzersiz bulunmuyor.</p>
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        <span className="text-sm text-muted-foreground">
          Egzersiz {currentIndex + 1} / {exercises.length}
        </span>
            ...
            {currentExercise.options.map((option, index) => (
              <Button
                ...
                className={cn(
                  ...
                  selectedOption === option && !showResult && "border-primary",
                  showResult && option === currentExercise.answer && "bg-green-500/20 border-green-500",
                  showResult && selectedOption === option && option !== currentExercise.answer && "bg-red-500/20 border-red-500"
                )}
        ...
        <div className={cn(
          ...
          {isCorrect ? (
            ...
            <div>
              ...
              <p className="text-sm">
                Dogru cevap: <span className="font-semibold text-green-400">{currentExercise.answer}</span>
              </p>
            </div>
          )}
...
function GrammarPractice({ 
  ...
  const allExercises = rules.flatMap((rule, ruleIndex) => 
    rule.exercises.map((exercise, exerciseIndex) => ({
      ...exercise,
  ...
  if (allExercises.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground">Secili filtrelere uygun egzersiz bulunamadi</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <Badge className={levelColors[currentExercise.ruleLevel]}>{currentExercise.ruleLevel}</Badge>
        ...
        <CardContent className="pt-6">
              ...
              {currentExercise.options.map((option, index) => (
                <Button
                  ...
                  className={cn(
                    ...
                    selectedOption === option && !showResult && "border-primary bg-primary/10",
                    showResult && option === currentExercise.answer && "bg-green-500/20 border-green-500",
                    showResult && selectedOption === option && option !== currentExercise.answer && "bg-red-500/20 border-red-500"
                  )}
                  onClick={() => !showResult && setSelectedOption(option)}
          ...
          <div className="flex gap-2 mt-6">
            {!showResult ? (
              ...
              <Button onClick={nextExercise} className="flex-1">
                {currentExerciseIndex < totalExercises - 1 ? 'Sonraki Soru' : 'Sonuclari Gor'}
                <ChevronRight className="h-4 w-4 ml-2" />
              </Button>
            )}
          </div>
