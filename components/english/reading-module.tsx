...
import { BookText, Clock, CheckCircle2, XCircle, ChevronRight, Volume2, Bookmark, RotateCcw } from 'lucide-react'
...
import type { Level, ReadingMaterial } from '@/lib/english-types'
...
Remember, you don't have to be perfect. Missing one day won't ruin your progress. What matters is getting back on track. The most successful people aren't those who never fail—they're the ones who never give up.`,
    level: 'B1', category: 'Self-Improvement', wordCount: 195, readingTime: 3,
    vocabulary: ['countless', 'consistent', 'automatic', 'foundation', 'resistance'],
    questions: [
      { question: 'What do successful people have in common according to the text?', options: ['They sleep late', 'They share common habits like waking up early and exercising', 'They avoid reading books', 'They change everything at once'], answer: 1 },
      { question: 'What do experts suggest for building good habits?', options: ['Change all habits at once', 'Start with the most difficult habits', 'Start small and focus on one habit at a time', 'Avoid setting goals'], answer: 2 },
      { question: 'What happens when good habits become automatic?', options: ['You become lazy', 'You free up mental energy for more important decisions', 'You stop making progress', 'You need to work harder'], answer: 1 }
    ]
  },
  {
    id: '2',
...
The transition to a sustainable future will require cooperation at all levels—from individuals to governments to international organizations. The technology exists; what we need now is the collective will to implement change before it's too late.`,
    level: 'B2', category: 'Environment', wordCount: 218, readingTime: 4,
    vocabulary: ['unprecedented', 'consequences', 'deforestation', 'sustainable', 'transition'],
    questions: [
      { question: 'What is the main cause of climate change mentioned in the text?', options: ['Solar radiation', 'The burning of fossil fuels', 'Volcanic eruptions', 'Ocean currents'], answer: 1 },
      { question: 'Which of the following is NOT mentioned as a consequence of climate change?', options: ['Melting ice caps', 'Rising sea levels', 'More earthquakes', 'Extreme weather events'], answer: 2 },
      { question: 'What does the text suggest about individual actions?', options: ['They are meaningless', 'Only governments can make a difference', 'Simple changes can make a difference', 'We should wait for new technology'], answer: 2 }
    ]
  },
  {
    id: '3',
...
Finally, adapt your communication style to your audience. What works with friends might not work in a business meeting. Consider cultural differences, personal preferences, and the context of your conversation. The best communicators are flexible and responsive to the needs of their listeners.`,
    level: 'B2', category: 'Communication', wordCount: 224, readingTime: 4,
    vocabulary: ['underrated', 'empathy', 'posture', 'contradict', 'jargon'],
    questions: [
      { question: 'What does the text say about listening?', options: ['It is the most practiced skill', 'Most people listen to respond rather than understand', 'It is not important for communication', 'It should be avoided in difficult conversations'], answer: 1 },
      { question: 'According to the text, what does crossing your arms signal?', options: ['Confidence', 'Interest', 'Defensiveness', 'Agreement'], answer: 2 },
      { question: 'What advice does the text give about communication style?', options: ['Always use the same style', 'Use complex language to impress', 'Adapt your style to your audience', 'Avoid pauses when speaking'], answer: 2 }
    ]
  }
]
...
export function ReadingModule() {
  ...
  const filteredMaterials = readingMaterials.filter(m => selectedLevel === 'all' || m.level === selectedLevel)
  ...
  return (
    <div className="space-y-6">
      ...
      {activeReading ? (
        <ReadingDetail material={activeReading} onBack={() => setActiveReading(null)} levelColor={levelColors[activeReading.level]} />
      ) : (
        <>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredMaterials.map((material) => (
              <Card key={material.id} className="cursor-pointer transition-all hover:shadow-lg hover:shadow-primary/5 hover:border-primary/50" onClick={() => setActiveReading(material)}>
                <CardContent className="pt-6">
                  ...
                  <p className="text-sm text-muted-foreground line-clamp-2 mb-4">{material.content.substring(0, 150)}...</p>
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    <span className="flex items-center gap-1"><BookText className="h-4 w-4" />{material.wordCount} kelime</span>
                    <span className="flex items-center gap-1"><Clock className="h-4 w-4" />{material.readingTime} dk</span>
                  </div>
...
function ReadingDetail({ material, onBack, levelColor }: { material: ReadingMaterial; onBack: () => void; levelColor: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Badge variant="outline">{material.category}</Badge>
        <span className="text-sm text-muted-foreground flex items-center gap-1"><BookText className="h-4 w-4" />{material.wordCount} kelime</span>
        <span className="text-sm text-muted-foreground flex items-center gap-1"><Clock className="h-4 w-4" />{material.readingTime} dk</span>
      </div>
      ...
      {!showQuiz ? (
        <>
          <Card>
            <CardContent className="pt-6">
              <div className="flex justify-end mb-4">
                <Button variant="ghost" size="sm" onClick={() => speakText(material.content)}><Volume2 className="h-4 w-4 mr-2" />Dinle</Button>
              </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between cursor-pointer" onClick={() => setShowVocabulary(!showVocabulary)}>
                <span className="flex items-center gap-2"><Bookmark className="h-5 w-5" />Anahtar Kelimeler</span>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {material.vocabulary.map((word, i) => (
                    <Badge key={i} variant="secondary" className="cursor-pointer hover:bg-primary/20" onClick={() => speakText(word)}>
                      {word}<Volume2 className="h-3 w-3 ml-1" />
                    </Badge>
                  ))}
                </div>
              </CardContent>
          ...
          <Button onClick={() => setShowQuiz(true)} className="w-full">Anlama Testine Basla<ChevronRight className="h-4 w-4 ml-2" /></Button>
        </>
      ) : (
        <Card>
          <CardContent className="pt-6">
            {!quizComplete ? (
                ...
                <div className="flex gap-2 mt-6">
                  {!showResult ? (
                    ...
                    <Button onClick={handleNextQuestion} className="flex-1">
                      {currentQuestion < material.questions.length - 1 ? 'Sonraki Soru' : 'Sonuclari Gor'}<ChevronRight className="h-4 w-4 ml-2" />
                    </Button>
                  )}
                </div>
              </>
              ...
              <div className="text-center">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => setShowQuiz(false)} className="flex-1">Metne Don</Button>
                  <Button onClick={() => { setShowQuiz(false); setCurrentQuestion(0); setSelectedAnswer(null); setShowResult(false); setScore(0); setQuizComplete(false); }} className="flex-1"><RotateCcw className="h-4 w-4 mr-2" />Tekrar Dene</Button>
                </div>
              </div>
            )}
          </CardContent>
